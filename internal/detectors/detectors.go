...
func Default() Set {
	return Set{
		detectors: []Detector{
			...
			newRegexDetector("rubygems_api_key", regexp.MustCompile(`\brubygems_[a-f0-9]{48}\b`), 0, ConfidenceHigh, nil),
			newRegexDetector("tailscale_api_key", regexp.MustCompile(`\btskey-[A-Za-z0-9_-]{20,}\b`), 0, ConfidenceHigh, nil),
			newKeyValueDetector("heroku_api_key",
				regexp.MustCompile(`(?i)heroku[_-]?(?:api[_-]?)?(?:key|token)`),
				regexp.MustCompile(`[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}`),
				ConfidenceHigh, nil),
			newKeyValueDetector("snyk_api_token",
				regexp.MustCompile(`(?i)snyk[_-]?(?:api[_-]?)?token`),
				regexp.MustCompile(`[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}`),
				ConfidenceHigh, nil),
			newKeyValueDetector("circleci_token",
				regexp.MustCompile(`(?i)circle(?:ci)?[_-]?(?:api[_-]?)?token`),
				regexp.MustCompile(`[a-f0-9]{40}`),
				ConfidenceHigh, nil),
			newKeyValueDetector("atlassian_api_token",
				regexp.MustCompile(`(?i)(?:atlassian|jira|confluence)[_-]?api[_-]?token`),
				regexp.MustCompile(`[A-Za-z0-9]{20,}`),
				ConfidenceMedium, looksLikeAssignedSensitiveValue),
			newKeyValueDetector("elastic_api_key",
				regexp.MustCompile(`(?i)elastic(?:search)?[_-]?(?:cloud[_-]?)?api[_-]?key`),
				regexp.MustCompile(`[A-Za-z0-9+/]{20,}`),
				ConfidenceHigh, looksLikeAssignedSensitiveValue),
			newKeyValueDetector("fastly_api_token",
				regexp.MustCompile(`(?i)fastly[_-]?(?:api[_-]?)?(?:token|key)`),
